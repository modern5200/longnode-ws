...
uuid: ${UUID}`;
  ...
  try {
    exec(command, { shell: '/bin/bash' }, (err) => {
      if (err) 控制台.error('npm running error:', err);
      else 控制台.log('npm is running');
    });
  } catch (error) {
    控制台.error(`error: ${error}`);
  }
};
...
async function addAccessTask() {
  ...
  try {
    ...
    控制台.log('Automatic Access Task added successfully');
  } catch (error) {
...
const delFiles = () => {
  ['npm', 'config.yaml'].forEach(文件 => fs.unlink(文件, () => { }));
};
...
httpServer.listen(PORT, () => {
  ...
  addAccessTask();
  控制台.log(`Server is running on port ${PORT}`);
});
